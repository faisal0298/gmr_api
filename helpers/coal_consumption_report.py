...
def header_data_value(template_data, month_date):
    try:
        ...
        header_data += "<h1 style='color: #000; margin: 0; font-size: 15px; text-align:center;'>GMR Warora Energy Limited</h1>"
        header_data += "<h1 style='color: #3a62ff; font-size: 12px; text-align:center;'>Daily Specific Coal Consumption Report</h1><br><br>"
        header_data += "</div>"
...
def generate_html_table(unit_name, unit_data):
    try:
        ...
        per_data += f"<td class='logic_table_td' style='text-align: center; font-size: 8px;'><strong>{round(unit_data['average_generation'], 2)}</strong></td>"
        per_data += f"<td class='logic_table_td' style='text-align: center;font-size: 8px;'><strong>{round(unit_data['average_consumption'], 2)}</strong></td>"
        per_data += f"<td class='logic_table_td' style='text-align: center;font-size: 8px;'><strong>{round(unit_data['average_specific_coal'], 2)}</strong></td></tr>"
