...
def header_data_value(template_data, month_date):
    try:
        ...
        header_data += "<h1 style='color: #3a62ff; margin: 2px auto; font-size: 15px;text-align:center;'>Coal GCV comparison Analysis Report</h1><br><br>"
        header_data += "</div>"
...
def generate_html_table(fetchtableData):
    try:
        per_data = ""
        ...
        per_data += "<th class='logic_table_th' rowspan='2' style='font-size: 8px;'>Year</th>"
        per_data += "<th class='logic_table_th' rowspan='2' style='font-size: 8px;'>Year-Month</th>"
        per_data += "<th class='logic_table_th' colspan='5' style='font-size: 8px;'>Coal Receipt</th>"
        per_data += "<th class='logic_table_th' colspan='4' style='font-size: 8px;'>Bunker Data</th>"
        per_data += "<th class='logic_table_th' colspan='2' style='font-size: 8px;'>Difference in GCV</th>"
        per_data += "</tr>"
        per_data += "<tr>"
        per_data += "<th class='logic_table_th' style='font-size: 8px;'>Domestic Qty</th>"
        per_data += "<th class='logic_table_th' style='font-size: 8px;'>Imported Qty</th>"
        per_data += "<th class='logic_table_th' style='font-size: 8px;'>Imported GCV</th>"
        per_data += "<th class='logic_table_th' style='font-size: 8px;'>Weighted GCV (MTD)</th>"
        per_data += "<th class='logic_table_th' style='font-size: 8px;'>Weighted GCV (YTD)</th>"
        per_data += "<th class='logic_table_th' style='font-size: 8px;'>Domestic Qty</th>"
        per_data += "<th class='logic_table_th' style='font-size: 8px;'>Imported Qty</th>"
        per_data += "<th class='logic_table_th' style='font-size: 8px;'>Weighted GCV (MTD)</th>"
        per_data += "<th class='logic_table_th' style='font-size: 8px;'>Weighted GCV (YTD)</th>"
        per_data += "<th class='logic_table_th' style='font-size: 8px;'>MTD</th>"
        per_data += "<th class='logic_table_th' style='font-size: 8px;'>YTD</th>"
        per_data += "</tr></thead><tbody style='border: 1px solid gray;'>"
        for year, months in fetchtableData["coal_receipt"].items():
            for coal, bunker in zip(months, fetchtableData["bunker_data"][year]):
                gcv_mtd = fetchtableData["gcv_difference"]["mtd"][year][coal["yearMonth"]]
                gcv_ytd = fetchtableData["gcv_difference"]["ytd"][year][coal["yearMonth"]]
                per_data += f"<tr style='height: 5px;'>"
                per_data += f"<td class='logic_table_td' style='text-align: center;'><span style='font-size: 8px; font-weight: 600;'> {coal['year']}</span></td>"
                per_data += f"<td class='logic_table_td' style='text-align: center;'><span style='font-size: 8px; font-weight: 600;'> {coal['yearMonth']}</span></td>"
                per_data += f"<td class='logic_table_td' style='text-align: center;'><span style='font-size: 8px; font-weight: 600;'> {coal['total_rR_Qty']}</span></td>"
                per_data += f"<td class='logic_table_td' style='text-align: center;'><span style='font-size: 8px; font-weight: 600;'> {coal['imported_qty']}</span></td>"
                per_data += f"<td class='logic_table_td' style='text-align: center;'><span style='font-size: 8px; font-weight: 600;'> {coal['imported_gcv']}</span></td>"
                per_data += f"<td class='logic_table_td' style='text-align: center;'><span style='font-size: 8px; font-weight: 600;'> {coal['average_arb_GCV']}</span></td>"
                per_data += f"<td class='logic_table_td' style='text-align: center;'><span style='font-size: 8px; font-weight: 600;'> {coal['annually_gcv']}</span></td>"
                per_data += f"<td class='logic_table_td' style='text-align: center;'><span style='font-size: 8px; font-weight: 600;'> {bunker['total_rR_Qty']}</span></td>"
                per_data += f"<td class='logic_table_td' style='text-align: center;'><span style='font-size: 8px; font-weight: 600;'> {bunker['imported_qty']}</span></td>"
                per_data += f"<td class='logic_table_td' style='text-align: center;'><span style='font-size: 8px; font-weight: 600;'> {bunker['average_ReceivedBasis_GCV']}</span></td>"
                per_data += f"<td class='logic_table_td' style='text-align: center;'><span style='font-size: 8px; font-weight: 600;'> {bunker['annually_gcv']}</span></td>"
                per_data += f"<td class='logic_table_td' style='text-align: center; background-color: yellow;'><span style='font-size: 8px; font-weight: 600;'> {gcv_mtd}</span></td>"
                per_data += f"<td class='logic_table_td' style='text-align: center; background-color: yellow;'><span style='font-size: 8px; font-weight: 600;'> {gcv_ytd}</span></td>"
                per_data += "</tr>"
...
def generate_report_comparision(specified_date, fetchtableData):
    try:
        payload={}
        header={}
        ...
        fetchTabledata = generate_html_table(fetchtableData)
            ...
            <div class="footertable" style="width:100%;margin-top:0px;">
                <div class="title" style="width: 100%; display: flex ; flex-direction:row; gap: 10px; height:20px; align-items:center">
                    <p style="color: #3a62ff; font-size: 12px; margin: 0px; font-weight: 600;">
                        Coal GCV comparison Analysis (FIFO)
                    </p>
                </div>
                        {fetchTabledata}
            </div>
        ...
        HTML(string=html_template).write_pdf(
            f"{store_data}/coal_gcv_comparision_analysis_report_{image_name}.pdf",
            stylesheets=[CSS(os.path.join(os.getcwd(), "helpers", "consumptionstyle.css"))],
        )
        # normal pdf name 
        pdf_file = f"{store_data}/coal_gcv_comparision_analysis_report_{image_name}.pdf"
        # watermark pdf name
        output_file = f"{store_data}/coal_gcv_comparision_analysis_report__{image_name}.pdf"
        data = {}
        if template_data.get("logo_as_watermark") == "title":
            ...
            data_file = f"static_server/gmr_ai/{file}/coal_gcv_comparision_analysis_report__{image_name}.pdf"
            # deleting original generated pdf as we are getting here two pdf first without watermark and second with watermark
            os.remove(f"{store_data}/coal_gcv_comparision_analysis_report_{image_name}.pdf")
            ...
            data_file = f"static_server/gmr_ai/{file}/coal_gcv_comparision_analysis_report_{image_name}.pdf"
